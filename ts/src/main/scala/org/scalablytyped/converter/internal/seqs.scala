...
import scala.collection.{BuildFrom, GenIterable, IterableOps}

object seqs {
  @inline final implicit class TraversableOps[C[t] <: Iterable[t] with IterableOps[t, C, _], T](private val ts: C[T])
      extends AnyVal {
    ...
    def partitionCollect[T1](t1: PartialFunction[T, T1])(
        implicit cbfT:           BuildFrom[C[T], T, C[T]],
        cbfT1:                   BuildFrom[C[T], T1, C[T1]],
    ): (C[T1], C[T]) = {
